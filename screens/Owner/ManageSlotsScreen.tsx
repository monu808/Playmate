...
import { getOwnerTurfs } from '../../lib/firebase/owner';
import {
  getUnavailableSlots,
  createBlockedSlot,
  deleteBlockedSlot,
  getAllBlockedSlotsForTurf,
} from '../../lib/firebase/firestore';
import { TIME_SLOTS } from '../../lib/constants';
import { formatTime } from '../../lib/utils';

export default function ManageSlotsScreen({ navigation }: any) {
  ...
  const loadSlots = useCallback(async () => {
    ...
    try {
      ...
      const { bookings, blockedSlots: blocked } = await getUnavailableSlots(selectedTurf.id, dateStr);
  ...
  const handleLockSlots = async () => {
    ...
    try {
      ...
      const slotData: any = {
        ...
        blockedBy: user.uid,
        blockedByName: userData?.displayName || user.email || 'Owner',
      };
  ...
  const renderTimeSlot = (slot: { startTime: string; endTime: string }, index: number) => {
    ...
    const isEndSelected = selectedEndTime && slot.startTime === selectedEndTime.split(':').map((v, i) => i === 0 ? String(Number(v) - 1).padStart(2, '0') : v).join(':');
    const isInRange = selectedStartTime && selectedEndTime &&
      slot.startTime >= selectedStartTime && slot.startTime < selectedEndTime;
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>Add a turf first to manage slots</Text>
        </View>
      </SafeAreaView>
    );
  }
